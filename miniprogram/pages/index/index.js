...
Page({
  data: {
    ...
    roofMaterialIndex: 0,
    roofThickness: 100,
    roofThicknessIndex: 2,
    roofK: 0.225,
    roofAlpha: 1.3,
    roofAlphaIndex: 2,
    roofAlphaOptions: ['无（室内顶棚）', '轻度日照', '中度日照（推荐）', '强烈日照'],
    roofAlphaValues: [1.0, 1.2, 1.3, 1.4],
    roofArea: 200,
    ...
    wallMaterialIndex: 0,
    wallThickness: 100,
    wallThicknessIndex: 2,
    wallK: 0.225,
    wallAlpha: 1.0,
    wallAlphaIndex: 0,
    wallAlphaOptions: ['标准（无强日照）', '轻度日照', '中度日照'],
    wallAlphaValues: [1.0, 1.1, 1.2],
    wallArea: 360,
    ...
    floorThickness: 100,
    floorThicknessIndex: 2,
    floorK: 0.225,
    floorType: 0.5,
    floorTypeIndex: 0,
    floorTypeOptions: ['架空地板（半温差）', '直接接触土壤', '下方为常温空间', '下方为加热空间'],
    floorTypeValues: [0.5, 0.7, 1.0, 0.3],
    floorArea: 200,

    // ===== 围护结构 - 隔墙 =====
    partitionArea: 0,
    partitionMaterial: 'pu',
    partitionMaterialIndex: 0,
    partitionThickness: 100,
    partitionThicknessIndex: 2,
    partitionK: 0.225,
    adjacentTemp: 0,
  ...
  onInput(e) {
    ...
    // 保存原始字符串，避免输入"1."时被parseFloat重置导致无法输入小数
  ...
  onMaterialChange(e) {
    ...
    const material = this.data.materialOptions[idx].value;
    this.setData({
      [`${part}MaterialIndex`]: idx,
      [`${part}Material`]: material
    });
    this.updatePartKValue(part);
    this.calculateAll();
  },
  ...
  onRoofAlphaChange(e) {
    ...
    this.setData({
      roofAlphaIndex: idx,
      roofAlpha: this.data.roofAlphaValues[idx]
    });
    this.calculateAll();
  },
  ...
  onWallAlphaChange(e) {
    ...
    this.setData({
      wallAlphaIndex: idx,
      wallAlpha: this.data.wallAlphaValues[idx]
    });
    this.calculateAll();
  },
  ...
  getParams() {
    ...
    return {
      roomName: d.roomName,
      roomType: d.roomType,
      indoorTemp: num(d.indoorTemp),
      outdoorTemp: num(d.outdoorTemp),
      outdoorHumidity: num(d.outdoorHumidity),
      indoorHumidity: num(d.indoorHumidity),
      roomLength: num(d.roomLength),
      roomWidth: num(d.roomWidth),
      roomHeight: num(d.roomHeight),
      safetyFactor: num(d.safetyFactor),
      roofK: num(d.roofK),
      wallK: num(d.wallK),
      floorK: num(d.floorK),
      partitionK: num(d.partitionK),
      roofAlpha: num(d.roofAlpha),
      wallAlpha: num(d.wallAlpha),
      floorAlpha: num(d.floorType),
      partitionArea: num(d.partitionArea),
      adjacentTemp: num(d.adjacentTemp),
      roofMaterial: d.roofMaterial,
      roofThickness: num(d.roofThickness),
      wallMaterial: d.wallMaterial,
      wallThickness: num(d.wallThickness),
      floorMaterial: d.floorMaterial,
      floorThickness: num(d.floorThickness),
      partitionMaterial: d.partitionMaterial,
      partitionThickness: num(d.partitionThickness),
      goodsMass: num(d.goodsMass),
      goodsInTemp: num(d.goodsInTemp),
      goodsOutTemp: num(d.goodsOutTemp),
      freezePoint: num(d.freezePoint),
      ...
      lightingDensity: num(d.lightingDensity),
      lightingTime: num(d.lightingTime),
  ...
  calculateAll() {
    ...
    const envelopeRows = [
      {
        name: '屋顶/顶棚',
        area: result.envelope.roof.area.toFixed(2),
        material: calc.MATERIAL_NAMES[params.roofMaterial],
        thickness: result.envelope.roof.area > 0 ? params.roofThickness : '-',
        k: result.envelope.roof.k.toFixed(3),
        delta: result.envelope.roof.delta.toFixed(1),
        alpha: result.envelope.roof.alpha.toFixed(2),
        load: result.envelope.roof.load.toFixed(1)
      },
      {
        name: '外墙（四面）',
        area: result.envelope.wall.area.toFixed(2),
        material: calc.MATERIAL_NAMES[params.wallMaterial],
        thickness: result.envelope.wall.area > 0 ? params.wallThickness : '-',
        k: result.envelope.wall.k.toFixed(3),
        delta: result.envelope.wall.delta.toFixed(1),
        alpha: result.envelope.wall.alpha.toFixed(2),
        load: result.envelope.wall.load.toFixed(1)
      },
      {
        name: '地板',
        area: result.envelope.floor.area.toFixed(2),
        material: calc.MATERIAL_NAMES[params.floorMaterial],
        thickness: result.envelope.floor.area > 0 ? params.floorThickness : '-',
        k: result.envelope.floor.k.toFixed(3),
        delta: result.envelope.floor.delta.toFixed(1),
        alpha: result.envelope.floor.alpha.toFixed(2),
        load: result.envelope.floor.load.toFixed(1)
      },
      {
        name: '隔墙',
        area: result.envelope.partition.area.toFixed(2),
        ...
        k: result.envelope.partition.k.toFixed(3),
        delta: result.envelope.partition.delta.toFixed(1),
        alpha: result.envelope.partition.alpha.toFixed(2),
        load: result.envelope.partition.load.toFixed(1)
      }
    ];
    ...
    this.setData({
      ...
      r1: (loads[0] / 1000).toFixed(2),
      r2: (loads[1] / 1000).toFixed(2),
      r3: (loads[2] / 1000).toFixed(2),
      r4: (loads[3] / 1000).toFixed(2),
      ...
      rTotal: result.totalKW.toFixed(2),
      rDesign: result.designKW.toFixed(2),
  ...
  exportReport() {
    ...
    if (!data) {
      wx.showToast({ title: '请先完成计算', icon: 'none' });
      return;
    }
...
`;
    ...
    wx.setClipboardData({
      ...
      success: () => {
        wx.showModal({
          title: '报告已复制',
          content: '计算报告已复制到剪贴板，可粘贴到微信或备忘录中保存。',
          showCancel: false
        });
      }
    });
